...
class Captcha {
	...
	public $ext_num_type = '';
	public $ext_pixel = true;
	//干扰点
	public $ext_line = true;
	//干扰线
	public $ext_rand_y = true;
	//Y轴随机

	public function __construct($len = 4, $im_width = '', $im_height = 25) {
		// 验证码长度、图片宽度、高度是实例化类时必      需的数据
		$this -> len = $len;
		$im_width = $len * 15;
	...
	public function get_randnum() {
		...
		if (1 == $this -> ext_num_type) {
			$str = $an1;
		}
		if (2 == $this -> ext_num_type) {
			$str = $an2;
		}
		if (3 == $this -> ext_num_type) {
			$str = $an3;
		}
...
//end class
/**使用验证码类的方法：
 * $an = new Captcha(验证码长度,图片宽度,图片高度);
